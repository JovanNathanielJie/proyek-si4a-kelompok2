            ...
            <form action="{{ route('mata_pelajaran.store') }}" method="POST">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="kode_mapel" value="{{ old('kode_mapel') }}">
                        @error('kode_mapel')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="nama_mapel" value="{{ old('nama_mapel') }}">
                        @error('nama_mapel')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="hari_les" class="form-control">
                            ...
                            @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'] as $hari)
                                <option value="{{ $hari }}" {{ old('hari_les') == $hari ? 'selected' : '' }}>{{ $hari }}</option>
                            @endforeach
                        </select>
                        @error('hari_les')
                        <div class="text-danger">{{ $message }}</div>
